...
async function buildEmbeds(page: number) {
    const menu: Menu[] = ((await Promise.all(pubs[page].map(evaluatePub))).filter((x) => x) as Menu[]).map((m) => ({
        ...m,
        items: (m.items || []).map((i) => ({
            ...i,
            item: i.item
    ...
    const embeds: APIEmbed[] = menu.map((menu) => ({
        ...
        description: (menu.items && menu.items.length > 0
            ? menu.items
                  .map(
                      (item) =>
                          `- ${item.item.replace(/^(((.*?)([:,().\-;]))|([A-Z\s]+)(\s))/gmu, '**$5$3**$4$6')} ${
...
export async function menuCommand(
    ...
    collector.on('collect', (sub_interaction: ButtonInteraction) => {
        collector.stop();
        message.edit({ components: [] });
        menuCommand(sub_interaction);
    });
}
